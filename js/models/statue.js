...
export function loadAnimatedStatueAndPassCallback(gltfLoader, filename, scaleFactor, need_bounding_box, position_x_z, offset, rotation_y, animation_name, callback) {
    const baseGroup = new THREE.Group()
    const material = new THREE.MeshPhongMaterial( {color: 0xFFFFFF} );
    const top_measures = {width: 2.8, height: 0.1}
    const bottom_measures = {width: 3, height: 0.6}
